...
with col_pipeline:
    ...
    def get_step_state(step_name):
        if step_name in r and r[step_name]:
            return "done"
...
if run_btn:
    if not topic.strip():
        ...
        # We'll use the graph's stream to update the UI in real-time
        ...
        try:
            # Container for the progress log
            with st.status("🚀 Initializing research agents...", expanded=True) as status:
                for event in graph.stream(initial_state):
                    for node_name, output in event.items():
                        st.session_state.current_step = node_name
                        # Update results with whatever was returned
                        st.session_state.results.update(output)
                        
                        if node_name == "search":
                            status.update(label="🔍 Search completed. Analyzing results...")
                        elif node_name == "reader":
                            status.update(label="📄 Content scraped. Drafting report...")
                        elif node_name == "writer":
                            status.update(label="✍️ Report written. Running critique...")
...
if r and "report" in r:
    ...
    st.markdown('<div class="section-heading">Research Results</div>', unsafe_allow_html=True)
    ...
    if "feedback" in r:
        ...
        <div class="feedback-panel">
            <div class="panel-label green">🧐 Critic Evaluation</div>
        """, unsafe_allow_html=True)
        st.markdown(r["feedback"])
        st.markdown("</div>", unsafe_allow_html=True)

    # Raw outputs in expanders
    with st.expander("🛠️ Debug View: Raw Agent Outputs", expanded=False):
        if "search_results" in r:
            st.markdown("### Search Results")
            st.code(r["search_results"][:1000] + "...")
        if "scraped_content" in r:
            st.markdown("### Scraped Content")
            st.code(r["scraped_content"][:1000] + "...")
